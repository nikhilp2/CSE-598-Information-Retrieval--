...
public class ClusteringNew {

	final int N = 50;
	final int NUM_SUMMARIES = 10;
	ForwardIndexWithFreq forwardIndex;
	VectorViewerIDF vectorViewer;
	IndexReader reader; 
	
	public ClusteringNew() {
	...
	public static void main(String[] args) {
		ClusteringNew c = new ClusteringNew();
		...
		try {
	        ...
	        while (true) {
	        	...
	        	HashMap<Integer,Cluster> results = c.GetClusters(k, query, topN);
	        	...
	        	while (itr.hasNext()) {
	        		...
	        		Cluster cluster = results.get(clusterNum);
	        		System.out.println("Cluster #" + clusterNum);
	        		System.out.print("Cluster Summary: ");
	        		for (int i=0; i<cluster.summaries.length; i++)
        				System.out.print(cluster.summaries[i] + " ");
	        		System.out.println();
	        		for (int i=0; i<cluster.docs.length; i++) {
	        			System.out.println("\t" + cluster.docs[i].docId + "\t" + cluster.docs[i].value + "\t" + c.reader.document(cluster.docs[i].docId).getField("url").stringValue());
	        		}
	        	}
	        }
	...
	public HashMap<Integer,Cluster> GetClusters(int k, String[] query, Vector[] topN) throws IOException{
		...
		for (int i=0; i<k; i++) {
			...
			centroid = forwardIndex.getTerms(topN[randomNumbers.get(i)].docId);
		...
		// Get the most predominant keywords from each centroid and apply it as a label/summary to each belonging cluster
		ArrayList<TreeMap<String,Double>> sortedCentroids = new ArrayList<TreeMap<String,Double>>();
		for (int i=0; i<centroids.size(); i++) {
			ValueComparator bvc =  new ValueComparator(centroids.get(i));
			TreeMap<String,Double> sortedSingleCentroid = new TreeMap<String,Double>(bvc);
			sortedSingleCentroid.putAll(centroids.get(i));
		...
		HashMap<Integer,Cluster> retVal = new HashMap<Integer,Cluster>();
		Iterator<Integer> itr = clusters.keySet().iterator();
		while(itr.hasNext()) { 
			int clusterNum = itr.next();
			ArrayList<Integer> docsInCluster = clusters.get(clusterNum);
			...
			// Attach the summaries
			TreeMap<String,Double> sortedSingleCentroid = sortedCentroids.get(clusterNum);
			Iterator<String> strItr = sortedSingleCentroid.keySet().iterator();
			String[] summaries = new String[NUM_SUMMARIES];
			int i=0;
			while(strItr.hasNext()) {
				String term = strItr.next();
				
				// If term is not the same as the query, then it qualifies to be part of the summary
				boolean isInQuery = false;
				for (int j=0; j<query.length; j++) {
					if (query[j].equalsIgnoreCase(term)) {
						isInQuery = true;
						break;
					}
				}
				...
				// If all the summaries have been filled, end
				if (i==NUM_SUMMARIES)
					break;
			}
			
			Cluster cluster = new Cluster();
			cluster.docs = docsInClusterCopy;
			cluster.summaries = summaries;
			retVal.put(clusterNum, cluster);
		}
